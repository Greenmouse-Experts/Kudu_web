                ...
                {productsArr.map((item) => (
                    <div key={item.id} className="bg-white p-4 border rounded-lg relative">
                        <Link to={`/product/${item.id}`}>
                            ...
                            <button
                                className={`absolute top-2 right-2 px-2 py-1 text-xs rounded font-meduim text-white ${item.vendor && item.vendor.isVerified ? "bg-green-500" : "bg-red-500"
                                    }`}
                            >
                                {item.vendor && item.vendor.isVerified ? "Verified" : "Not Verified"}
                            </button>
                            <span
