...
export default function ViewKYC() {
  ...
  useEffect(() => {
    console.log(
      "🚀 ViewKYC Component initialized with ID:",
      id,
      "(type:",
      typeof id,
      ")"
    ...
    // More flexible ID validation - allow non-numeric IDs
    const trimmedId = String(id).trim();
    if (!trimmedId) {
      console.log("❌ Empty vendor ID provided:", id);
      setError("Invalid vendor ID provided");
      setIsLoading(false);
      return;
    }

    console.log("✅ Valid vendor ID provided:", trimmedId);
  }, [id]);
  ...
  const rejectKYC = (id) => {
    openModal({
      ...
      content: (
        <>
          <form
            onSubmit={handleSubmit((data) => submitKyc(id, data, "reject"))}
          >
            <div className="w-full flex flex-col h-auto px-3 py-6 gap-3 -mt-3">
              <div className="flex justify-center gap-5 w-full">
                <p className="font-semibold text-center text-lg">Admin Note</p>
              </div>
              <div>
                <input
                  type="text"
  ...
  const getKYC = () => {
    ...
    const trimmedId = String(id).trim();
    if (!trimmedId) {
      console.log("❌ Empty vendor ID after trimming:", id);
      setError("Invalid vendor ID provided");
      setIsLoading(false);
      return;
    }
    ...
    console.log(
      "🔍 Fetching KYC data for vendor ID:",
      trimmedId,
      "(original:",
      id,
      ", type:",
    ...
    mutate({
      ...
      onSuccess: (response) => {
        try {
          ...
          // Try multiple comparison methods with better logging
          let userKYC = response.data.data.find((item) => {
            console.log(
              `🔍 Comparing: ${
                item.vendorId
              } (${typeof item.vendorId}) with ${vendorIdString} (string) and ${vendorIdNumber} (number)`
            );
            ...
            if (
              ...
              console.log(
                "✅ Found match using case-insensitive string comparison"
              );
              return true;
            }
            return false;
  ...
  useEffect(() => {
    if (id) {
      console.log("🔄 useEffect triggered for ID:", id, "type:", typeof id);
      const trimmedId = String(id).trim();
      if (trimmedId) {
        console.log("✅ Valid ID, calling getKYC for:", trimmedId);
        getKYC();
      } else {
        console.log("❌ Invalid ID after trimming:", id);
        setError("Invalid vendor ID provided");
        setIsLoading(false);
      }
    } else {
      console.log("❌ No ID provided in useEffect:", id);
      setError("No vendor ID provided");
      setIsLoading(false);
    }
  }, [id]);
      ...
      <div className="w-full h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            This vendor has not completed their KYC process.
          </p>
          <button
            onClick={() => navigate("/admin/all-vendors")}
            className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600"
    ...
    <div className="bg-gray-50 min-h-screen p-6">
      <div className="max-w-4xl mx-auto">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
            ...
            >
              <path
                ...
                strokeWidth="2"
                d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-2m-2 0H7m-2 0H5m2 0v-9a2 2 0 012-2h2a2 2 0 012 2v9"
              />
            </svg>
            Business Information
          </h2>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          {/* Document Images */}
          {kycData.idVerification &&
            kycData.idVerification.prototypes &&
            kycData.idVerification.prototypes.length > 0 && (
              <div className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="p-3 bg-gray-50 border rounded-lg">
                ...
                >
                  {kycData?.isVerified ? "Verified" : "Not Verified"}
                </span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex space-x-4">
              <button
                onClick={() =>
                  kycData.id && submitKyc(kycData.id, null, "approve")
                }
